...
linelist <- linelist %>%
  mutate(
    ...
    gender_std = case_when(
      str_to_lower(str_trim(gender)) %in%
        c("male", "m", "man", "1", "boy") ~ "Male",
      str_to_lower(str_trim(gender)) %in%
        c("female", "f", "fem", "woman", "2", "girl") ~ "Female",
    ...
    sick_std = case_when(
      str_to_lower(as.character(sick)) %in% true_set ~ TRUE,
      str_to_lower(as.character(sick)) %in% false_set ~ FALSE,
      TRUE ~ NA
    ),
    diarrhoea = case_when(
      str_to_lower(as.character(diorpha)) %in% true_set ~ TRUE,
      str_to_lower(as.character(diorpha)) %in% false_set ~ FALSE,
      TRUE ~ NA
    ),
    abdominal_pain_std = case_when(
      str_to_lower(as.character(abdominal_pain)) %in% true_set ~ TRUE,
      str_to_lower(as.character(abdominal_pain)) %in% false_set ~ FALSE,
      TRUE ~ NA
    ),
    headache_std = case_when(
      str_to_lower(as.character(headache)) %in% true_set ~ TRUE,
      str_to_lower(as.character(headache)) %in% false_set ~ FALSE,
      TRUE ~ NA
    ),
    fatigue_std = case_when(
      str_to_lower(as.character(fatigue)) %in% true_set ~ TRUE,
...
linelist <- linelist %>%
  mutate(
    ...
    symptom_onset_dt = case_when(
      ...
      onset_year < 2025 | onset_year > 2026 ~ update(
        symptom_onset_parsed,
        year = 2026
      ),
      TRUE ~ symptom_onset_parsed
    ),
    hours_since_party = as.numeric(difftime(
      symptom_onset,
      party_dt,
      units = "hours"
    ))
...
or_table <- linelist %>%
  ...
  rowwise() %>%
  mutate(
    ft = list(suppressWarnings(fisher.test(matrix(
      c(a, b, c, d),
      nrow = 2,
      byrow = TRUE
    )))),
